...
satellites  = {'SMOS', 'SMAP'};

refSMs      = {'V5','5cm','10cm'};


for u1 = 1:length(domainids)
    for u2 = 1:length(satellites)
        for u3 = 1:length(refSMs)

            domainid   = domainids{u1};

            satellite  = satellites{u2};
            ...
            if savefigyes; saveas(gcf, [figdir, sprintf('%s_%s_rv_and_insitu_%s_timeseries_v%s.png', satellite, domainid, refSM, curdate)]); end
            ...
            if savefigyes; saveas(gcf, [figdir, sprintf('%s_%s_rh_and_insitu_%s_timeseries_v%s.png', satellite, domainid, refSM, curdate)]); end
            ...
            if savefigyes
                saveas(gcf, [figdir, sprintf('%s_%s_r_insitu_%s_scatter_v%s.png', satellite, domainid, refSM, curdate)]);
            end
